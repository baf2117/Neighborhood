...
namespace neighborhood
{
    ...
    public partial class RemotePost
    {
        ...
        public string Post()
        {
            ...
            Append(sb, "<html><head>");
            Append(sb, $"</head><body onload=\"document.{FormName}.submit()\">");
            if (!string.IsNullOrEmpty(AcceptCharset))
            {
                //AcceptCharset specified
                Append(sb, $"<form name=\"{FormName}\" method=\"{Method}\" action=\"{Url}\" accept-charset=\"{AcceptCharset}\">");
            }
            else
            {
                //no AcceptCharset specified
                Append(sb, $"<form name=\"{FormName}\" method=\"{Method}\" action=\"{Url}\" >");
            }
            if (NewInputForEachValue)
            {
                foreach (string key in Params.Keys)
                {
                    ...
                    if (values != null)
                    {
                        ...
                        {
                            Append(sb, $"<input name=\"{WebUtility.HtmlEncode(key)}\" type=\"hidden\" value=\"{WebUtility.HtmlEncode(value)}\">");
                        }
                    }
                }
            }
            else
            {
                for (var i = 0; i < Params.Keys.Count; i++)
                    Append(sb, $"<input name=\"{WebUtility.HtmlEncode(Params.Keys[i])}\" type=\"hidden\" value=\"{WebUtility.HtmlEncode(Params[Params.Keys[i]])}\">");
            }
            Append(sb, "</form>");
            Append(sb, "</body></html>");
